...
# 要件要約:
# - 入力順で「見積書1〜5」へ A=品名 / F=数量 / G=単位 / H=単価 を転記。11行目は触らず 12〜44 を使用（33行/頁）。
# - 1間口33行以内はページ跨ぎ禁止、33行超は跨ぎOK、総ページ>5はエラー。
# - 見積書0の 21〜44 行へ「間口合計」を連続記載（空行なし）。A=カーテン品名 / F=1 / G=式 / H=合計。
# - 「運賃・梱包」は見積書0の末尾に追記（明細には載せない）。S・MACまたはエア・セーブMAが含まれる場合は必須。
# - 見積番号は 3709-xxxxx（5桁乱数）固定。再生成ボタン無し。
# - テンプレは「お見積書（明細）.xlsx」：見積書0／見積書1〜5 がある場合は罫線/結合を保持して出力。
...
def _next_estimate_no() -> str:
    """見積番号 3709-xxxxx（5桁）。セッション内で重複回避。"""
...
def load_master_book(path: Path) -> Dict[str, List[Dict[str, Any]]]:
    """master.xlsx → {シート名: [{品名, 単位, 単価, 型番}, ...]}（1行目ヘッダ前提）。"""
...
def _header_section():
    ...
    with c1:
        st.text_input("見積番号（固定：3709-xxxxx）",
                      value=st.session_state.estimate_no,
                      key="estimate_no", disabled=True)
    with c2:
        st.text_input("保存ファイル名（接頭辞）",
                      value=st.session_state.file_title,
                      key="file_title")
    with c3:
        st.text_input("作成日",
    ...
    with d1:
        st.text_input("得意先名（見積書0:A6）",
                      value=st.session_state.customer_name,
                      key="customer_name")
    with d2:
        st.text_input("支店名（見積書0:A7 左）",
                      value=st.session_state.branch_name,
                      key="branch_name")
    with d3:
        st.text_input("営業所名（見積書0:A7 右）",
                      value=st.session_state.office_name,
                      key="office_name")
    with d4:
        st.text_input("担当者名（半角英数2〜4）",
                      value=st.session_state.rep_name,
                      key="rep_name")
        rep_cur = st.session_state.get("rep_name") or ""
        if rep_cur and not re.fullmatch(r"[A-Za-z0-9]{2,4}", rep_cur):
            st.error("担当者名は半角英数字2〜4文字で入力してください。")

    st.text_input("物件名（見積書0:B17）",
...
def _shipping_section():
    ...
    with c2:
        st.number_input("運賃・梱包 金額（円）", min_value=0, step=100,
...
def _save_section():
    ...
    if st.button("Excel保存（お見積書（明細））", key="excel_save_btn"):
        ...
        try:
            ...
            # テンプレが完全（見積書0／1〜5）なら罫線保持で出力、無ければ保険の単一シート法
            use_preserve = False
            try:
